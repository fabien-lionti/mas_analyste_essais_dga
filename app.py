...
from segment_cnn_model import MODEL_METADATA_PATH, MODEL_SUMMARY_PATH, MODEL_WEIGHTS_PATH, Simple1DCNN, load_model_summary, predict_file, generate_active_learning_cache, MODEL_PROGRESS_PATH, MODEL_DIR, write_json_atomic
...
NEURAL_ODE_TRAIN_PROGRESS_PATH = ANNOTATION_DIR / "neural_ode_training_progress.json"
NEURAL_ODE_TRAIN_LOG_PATH = ANNOTATION_DIR / "neural_ode_training.log"
DRIFT_RUN_PREFIXES = ("dynamic_channel_coherence", "neural_ode")
        ...
        return [
            {
                ...
                "open_error": "DXD brut present, JSON resample manquant. Lancer 00_scan_dxd_channel_presence.py.",
            }
...
def safe_neural_ode_output_dir(name: str) -> Path:
    clean = str(name or "").strip()
    if not clean:
        raise HTTPException(status_code=400, detail="Missing output_dir")
    if "/" in clean or "\\" in clean or clean in {".", ".."}:
        raise HTTPException(status_code=400, detail="output_dir must be a directory name, not a path")
    if not clean.startswith("neural_ode"):
        raise HTTPException(status_code=400, detail="output_dir must start with neural_ode")
    return BASE_DIR / clean
...
def api_drift_coherence_train(background_tasks: BackgroundTasks, payload: Dict[str, Any]):
    ...
    cmd = [
        sys.executable,
        str(BASE_DIR / "train_incremental_tcn_coherence.py"),
        "--input-dir",
        str(RAW_JSON_DIR),
        "--output-dir",
        str(output_dir),
...
@app.post("/api/neural-ode/train")
def api_neural_ode_train(background_tasks: BackgroundTasks, payload: Dict[str, Any]):
    if NEURAL_ODE_TRAIN_PROGRESS_PATH.exists():
        try:
            current = load_json(NEURAL_ODE_TRAIN_PROGRESS_PATH)
            if current.get("status") == "running":
                return {"status": "already_running", "message": "Un entraînement Neural ODE est déjà en cours."}
        except Exception:
            pass

    output_dir = safe_neural_ode_output_dir(str(payload.get("output_dir", "neural_ode_anomaly_run")))
    targets = payload.get("targets") or ["vehicle.ay", "vehicle.yaw_rate", "steer_s1", "steer_s2"]
    if isinstance(targets, str):
        targets = [t.strip() for t in targets.split(",") if t.strip()]
    if not isinstance(targets, list) or not targets:
        raise HTTPException(status_code=400, detail="targets must be a non-empty list")
    allowed_targets = {"vehicle.vx", "vehicle.ax", "vehicle.ay", "vehicle.yaw_rate", "steer_s1", "steer_s2"}
    targets = [str(t) for t in targets if str(t) in allowed_targets]
    if not targets:
        raise HTTPException(status_code=400, detail="No valid target selected")

    window_sec = numeric_payload_value(payload, "window_sec", 6.0, 0.2, 60.0)
    stride_sec = numeric_payload_value(payload, "stride_sec", 1.0, 0.05, 30.0)
    holdout_fraction = numeric_payload_value(payload, "holdout_fraction", 0.2, 0.0, 0.8)
    batch_size_files = integer_payload_value(payload, "batch_size_files", 50, 1, 500)
    epochs = integer_payload_value(payload, "epochs", 20, 1, 500)
    batch_size = integer_payload_value(payload, "batch_size", 64, 1, 2048)
    hidden_dim = integer_payload_value(payload, "hidden_dim", 64, 4, 1024)
    num_layers = integer_payload_value(payload, "num_layers", 2, 1, 10)
    rollout_steps = integer_payload_value(payload, "rollout_steps", 10, 1, 200)
    time_bin_minutes = integer_payload_value(payload, "time_bin_minutes", 60, 5, 1440)
    max_windows_per_file = payload.get("max_windows_per_file")
    if max_windows_per_file in ("", None):
        max_windows_per_file = None
    ...
    cmd = [
        sys.executable,
        str(BASE_DIR / "train_neural_ode_anomaly.py"),
        "--input-dir",
        str(RAW_JSON_DIR),
        "--output-dir",
        str(output_dir),
    ...
    started_at = datetime.utcnow().isoformat(timespec="seconds") + "Z"
    write_json_atomic(
        NEURAL_ODE_TRAIN_PROGRESS_PATH,
        {
            "status": "running",
            "started_at": started_at,
            "output_dir": output_dir.name,
            "targets": targets,
            "command": cmd,
            "log_path": str(NEURAL_ODE_TRAIN_LOG_PATH),
        },
    )
    ANNOTATION_DIR.mkdir(parents=True, exist_ok=True)
    NEURAL_ODE_TRAIN_LOG_PATH.write_text(
        f"[{started_at}] Starting Neural ODE anomaly training\n" + " ".join(cmd) + "\n\n",
        encoding="utf-8",
    )

    def run_training():
        try:
            with open(NEURAL_ODE_TRAIN_LOG_PATH, "a", encoding="utf-8") as log:
                proc = subprocess.run(
                    cmd,
                    cwd=str(BASE_DIR),
                    stdout=log,
...
@app.get("/api/neural-ode/train/progress")
def api_neural_ode_train_progress(tail_lines: int = 80):
    if NEURAL_ODE_TRAIN_PROGRESS_PATH.exists():
        try:
            progress = load_json(NEURAL_ODE_TRAIN_PROGRESS_PATH)
        except Exception as e:
            progress = {"status": "unknown", "message": str(e)}
    else:
        progress = {"status": "not_started"}
    if NEURAL_ODE_TRAIN_LOG_PATH.exists():
        try:
            lines = NEURAL_ODE_TRAIN_LOG_PATH.read_text(encoding="utf-8", errors="replace").splitlines()
            progress["log_tail"] = "\n".join(lines[-max(1, min(int(tail_lines), 500)):])
        except Exception as e:
            progress["log_tail"] = f"Unable to read log: {e}"
    else:
...
def api_window_quality(
    ...
    return deep_json_safe(
        {
            "status": "ok",
            "path": str(WINDOW_QUALITY_INDEX_PATH),
            "filters": {
                "json_name": json_name,
    ...
    if json_name:
        ...
        try:
            ...
            if lat_col and lon_col:
                ...
                if len(out) > 6000:
                    idx = np.linspace(0, len(out) - 1, 6000).astype(int)
                    out = out.iloc[idx]
                trajectory = {
                    "json_name": json_name,
...
def api_exploration_signal_boxplot(
    ...
    for file_name, file_segments in segments.groupby("json_name", sort=False):
        ...
        if signal not in df.columns:
            continue
        file_df = df[["time", signal]].dropna().copy()
        if file_df.empty:
            continue
        for seg in file_segments.itertuples(index=False):
            ...
            values = file_df[(file_df["time"] >= start) & (file_df["time"] <= end)].copy()
            if values.empty:
                continue
            remaining = max(0, int(max_points) - total_points)
            if len(values) > remaining:
                values = values.iloc[:remaining]
            total_points += int(len(values))
            duration_sec += max(0.0, end - start)
            values = values.rename(columns={signal: "value"})
            for row in values.itertuples(index=False):
                date_value, hour_value, bucket = time_bucket_for_value(raw, float(row.time), time_grouping)
                rows.append(
                    {
                        "json_name": str(file_name),
                        "date": date_value,
                        "hour": hour_value,
                        "time_bucket": bucket,
                        "label": str(seg.label),
                        "label_source": str(seg.label_source),
                        "signal": signal,
    ...
    for keys, group in values_df.groupby(group_cols, dropna=False, sort=True):
        stats = quantile_row(group["value"])
        if not stats or stats["n_points"] < min_points:
            continue
        row = {col: val for col, val in zip(group_cols, keys)}
        row.update(stats)
...
def api_train_segment_model(background_tasks: BackgroundTasks, payload: Optional[Dict[str, Any]] = None):
    ...
    def run_training():
        try:
            from segment_cnn_model import train_from_annotations, TrainingConfig
