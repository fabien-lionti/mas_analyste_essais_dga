...
def _dynamic_stat_values(dynamic_analysis: dict[str, Any], segment: dict[str, Any]) -> dict[str, Any]:
    tags = [
        item for item in (dynamic_analysis.get("indicators") or [])
        if isinstance(item, dict) and item.get("enabled", True)
    ]
    channels = segment.get("signal_context", {}).get("channels") or {}
    values_by_tag: dict[str, Any] = {}
    for tag in tags:
        if tag.get("source_kind", "channel") != "channel":
            continue
        source_name = tag.get("source_name")
        stat_name = tag.get("stat_name")
        tag_name = tag.get("tag_name")
        if not source_name or not stat_name or not tag_name:
            continue
        channel = channels.get(source_name) or {}
        values = [
            numeric
            for numeric in (numeric_or_none(value) for value in (channel.get("values") or []))
            if numeric is not None
        ]
        if not values:
            values_by_tag[tag_name] = None
            continue
        if stat_name == "min":
            value = min(values)
        elif stat_name == "max":
            ...
            value = statistics.pstdev(values) if len(values) > 1 else 0.0
        elif stat_name == "rms":
            value = (statistics.fmean([item * item for item in values])) ** 0.5
        elif stat_name == "amplitude":
            value = max(values) - min(values)
        else:
            value = None
        values_by_tag[tag_name] = value
    return values_by_tag


def _format_dynamic_stat_value(value: Any) -> str:
    if value is None:
...
def _segment_tag_values(segment: dict[str, Any]) -> dict[str, Any]:
    return {
        "segment.annotation_id": segment.get("annotation_id"),
        "segment.file_name": segment.get("file_name"),
        "segment.label": segment.get("label"),
        "segment.start_time_sec": segment.get("start_time_sec"),
        "segment.end_time_sec": segment.get("end_time_sec"),
        "segment.duration_sec": segment.get("duration_sec"),
    }


def _render_prompt_tags(text: str, tag_values: dict[str, Any]) -> str:
    def replace(match: re.Match[str]) -> str:
        tag_name = match.group(1).strip()
        if tag_name not in tag_values:
            if tag_name.startswith("segment."):
                return "non disponible"
            return match.group(0)
        return _format_dynamic_stat_value(tag_values.get(tag_name))
...
def generate_segment_artifact(
    *,
    ...
    try:
        ...
        axes = [ax]
        colors = ["#1368ce", "#15803d", "#e26d0a", "#8b5cf6", "#b42318", "#0f766e"]
        start = float(segment["start_time_sec"])
        end = float(segment["end_time_sec"])
        lines = []
        for index, (channel, payload) in enumerate(channel_items):
            current_ax = ax if index == 0 else ax.twinx()
            if index > 1:
                current_ax.spines["right"].set_position(("outward", 48 * (index - 1)))
            if index > 0:
                axes.append(current_ax)
            channel, payload = channel_items[index]
            raw_values = payload.get("values") or []
            points = [
                (numeric_or_none(time_value), numeric_or_none(value))
                for time_value, value in zip(time_values, raw_values)
            ]
            points = [
                (time_value, value)
            ...
            if points:
                ...
                lines.append(line)
            current_ax.axvspan(start, end, color="#f59e0b", alpha=0.10)
            unit = payload.get("unit") or ""
            current_ax.set_ylabel(
                f"{channel}{f' ({unit})' if unit else ''}",
                fontsize=8,
                color=colors[index % len(colors)],
            )
            current_ax.tick_params(axis="y", labelsize=8, colors=colors[index % len(colors)])
            current_ax.spines["right" if index else "left"].set_color(colors[index % len(colors)])
            current_ax.grid(index == 0, alpha=0.25)
        ax.set_title(f"{segment.get('label') or ''} - {segment.get('file_name') or segment['annotation_id']}")
        ax.set_xlabel("Temps (s)")
        if lines:
            ax.legend(lines, [line.get_label() for line in lines], loc="upper right", fontsize=8, frameon=False)
        right_bound = max(0.58, 0.88 - 0.065 * max(0, len(channel_items) - 2))
        fig.tight_layout(rect=[0, 0, right_bound, 1])
...
def dynamic_llm_prompt(dynamic_analysis: dict[str, Any], segment: dict[str, Any], user_prompt: str) -> tuple[str, dict[str, Any]]:
    stat_values = _dynamic_stat_values(dynamic_analysis, segment)
    segment_values = _segment_tag_values(segment)
    prompt_tag_values = {**segment_values, **stat_values}
    context = {
        "dynamic_analysis": {
            ...
            "stat_tags": dynamic_analysis.get("indicators") or [],
        },
        "segment": {
            "annotation_id": segment.get("annotation_id"),
            "file_name": segment.get("file_name"),
        ...
        "segment_tag_values": segment_values,
        "stat_values": stat_values,
    }
    prompt_parts = [
        _render_prompt_tags(dynamic_analysis.get("system_prompt") or "", prompt_tag_values).strip(),
        _render_prompt_tags(user_prompt, prompt_tag_values).strip(),
    ]
...
def create_prediction_for_segment(
    ...
    *,
    ...
    stat_values = _dynamic_stat_values(dynamic_analysis, segment)
    segment_values = _segment_tag_values(segment)
    llm_prompt, prompt_context = dynamic_llm_prompt(dynamic_analysis, segment, payload.user_prompt)
    ...
    if payload.provider == "gemini":
        ...
        if stat_values:
            analysis_lines.append("Balises statistiques:")
            for tag_name, value in stat_values.items():
                analysis_lines.append(f"{{{{{tag_name}}}}}: {_format_dynamic_stat_value(value)}")
        parts = {
            "analysis_text": "\n".join(analysis_lines),
    ...
    if payload.provider == "gemini":
        response_json["provider"] = {
            "name": "gemini",
            "model": model,
            "usage": gemini.get("raw_usage"),
            "finish_reason": gemini.get("finish_reason"),
        }
...
def context_payload_for_dynamic_analysis(
    dynamic_analysis: dict[str, Any],
    payload: DynamicPredictionRequest,
) -> DynamicContextRequest:
    return DynamicContextRequest(
...
def dynamic_segment_preview(
    *,
    ...
    prompt, prompt_context = dynamic_llm_prompt(dynamic_analysis, segment, payload.user_prompt)
    return {
        "annotation_id": segment["annotation_id"],
        "status": "preview",
        "updated_at": now_iso(),
        ...
        "input_context": {
            ...
            "request": context["request"],
            "segment": segment,
            "segment_tag_values": prompt_context["segment_tag_values"],
            "stat_values": prompt_context["stat_values"],
            "context": prompt_context["context"],
            "prompt": prompt,
        },
    }
...
def create_dynamic_predictions_endpoint(
    ...
    if dynamic_analysis is None:
        raise HTTPException(status_code=404, detail="Dynamic analysis not found")
    context_payload = context_payload_for_dynamic_analysis(dynamic_analysis, payload)
...
@router.post("/{dynamic_analysis_id}/preview/{annotation_id}")
def preview_dynamic_prediction_for_annotation_endpoint(
    analysis_id: str,
    dynamic_analysis_id: str,
    annotation_id: str,
    payload: DynamicPredictionRequest,
