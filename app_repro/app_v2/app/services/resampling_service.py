...
def should_convert_export_values(
    *,
    requested: str,
    source_name: str,
    display_name: str,
    ...
    if spec is None or spec.scale == 1.0:
        return False
    channel_unit = normalized_unit(str(getattr(channel, "unit", "") or ""))
    if channel_unit:
        if channel_unit == normalized_unit(spec.unit):
            return False
        if channel_unit == normalized_unit(spec.raw_unit):
            return True
    if display_name == lookup_name or source_name == lookup_name:
        return True
    if source_name == canonical or requested == canonical:
        return False
    return True
...
def export_dxd_file_to_json(
    *,
    ...
    with reader_factory(path) as reader:
        ...
        source_series: list[tuple[str, str, str, pd.Series, Any]] = []
        ...
        for requested, channel in resolved:
            ...
            source_series.append((requested, str(getattr(channel, "name", "")), str(channel), series, channel))
        ...
        for requested, source_name, display_name, series, channel in source_series:
            ...
            if should_convert_export_values(
                requested=requested,
                ...
                values = convert_channel_values(lookup_name, values)
