...
from app_v2.app.api.routes.dynamic_analysis import (
    ...
    preview_dynamic_prediction_for_annotation_endpoint,
...
from app_v2.app.api.routes.exploration import exploration_boxplot_endpoint, exploration_parametric_endpoint
...
def test_exploration_boxplot_returns_time_buckets(tmp_path: Path):
    db_path = tmp_path / "api.sqlite"
    dxd_dir = tmp_path / "campaign"
    json_dir = tmp_path / "json"
    dxd_dir.mkdir()
    ...
    json_path.write_text(
        """
        {
          "timebase": {"time": [0, 10, 20, 30]},
          "channels": {
            "resampled": {
              "WheelSteer_S1 (_)": {"values": [1, 3, 5, 7], "unit": "deg"}
            }
          }
        }
        """,
    ...
    with connect(db_path) as conn:
        init_db(conn)
        analysis = create_analysis_endpoint(
            CreateAnalysisRequest(name="Quality", source_dxd_dir=str(dxd_dir)),
            conn,
        )
        analysis_id = analysis["analysis_id"]
        discover_dxd_endpoint(analysis_id, DiscoverDxdRequest(dxd_dir=str(dxd_dir)), conn)
        ...
        payload = exploration_boxplot_endpoint(
            analysis_id,
            signal="WheelSteer_S1 (_)",
        ...
        assert payload["coverage"]["n_files"] == 1
        assert payload["coverage"]["n_points"] == 4
        assert len(payload["items"]) == 1
        assert payload["items"][0]["time_bucket"] == "2025-04-24T10:00:00"
        assert payload["items"][0]["q50"] == 4
        assert payload["items"][0]["n_points"] == 4

        create_managed_annotation_label_endpoint(
            analysis_id,
            ManagedAnnotationLabelRequest(name="ligne droite - vitesse constante"),
            conn,
        )
        create_annotation_endpoint(
            analysis_id,
            AnnotationWriteRequest(
                ...
                start_time_sec=10,
                end_time_sec=20,
                label="ligne droite - vitesse constante",
            ),
            conn,
        )
        filtered = exploration_boxplot_endpoint(
            analysis_id,
            signal="WheelSteer_S1 (_)",
            label="ligne droite - vitesse constante",
            time_grouping="day_hour",
        ...
        assert filtered["coverage"]["n_files"] == 1
        assert filtered["coverage"]["n_points"] == 2
        assert filtered["filters"]["label"] == "ligne droite - vitesse constante"
        assert filtered["items"][0]["q50"] == 4

        day_payload = exploration_boxplot_endpoint(
            analysis_id,
            signal="WheelSteer_S1 (_)",
            time_grouping="day",
            min_points=1,
        ...
        assert len(day_payload["items"]) == 1
        assert day_payload["items"][0]["time_bucket"] == "2025-04-24"
        assert day_payload["items"][0]["n_points"] == 4
...
def test_dynamic_analysis_definition_prediction_and_correction(tmp_path: Path):
    ...
    with connect(db_path) as conn:
        ...
        overwritten_dynamic_analysis = create_dynamic_analysis_endpoint(
            ...
            DynamicAnalysisDefinitionRequest(
                name=" Analyse freinage ",
                system_prompt="Expert dynamique mis a jour. Label: {{segment.label}}. Ay max: {{stat.vehicle.ay.max}}.",
                selected_channels=["vehicle.ay"],
                selected_labels=["freinage"],
                indicators=[
                    {
                        "source_kind": "channel",
                        "source_name": "vehicle.ay",
                        "stat_name": "max",
                        "tag_name": "stat.vehicle.ay.max",
                        "enabled": True,
                    }
                ],
            ),
        ...
        assert "Label: {{segment.label}}" in overwritten_dynamic_analysis["system_prompt"]
        assert "Ay max: {{stat.vehicle.ay.max}}" in overwritten_dynamic_analysis["system_prompt"]
        assert overwritten_dynamic_analysis["selected_channels"] == ["vehicle.ay"]
        assert overwritten_dynamic_analysis["indicators"][0]["tag_name"] == "stat.vehicle.ay.max"
        assert [item["name"] for item in listed_dynamic_analyses].count("Analyse freinage") == 1

        preview = preview_dynamic_prediction_for_annotation_endpoint(
            analysis_id,
        ...
        assert preview["input_artifact_path"]
        assert preview["input_context"]["stat_values"]["stat.vehicle.ay.max"] == 1
        assert "Label: freinage" in preview["input_context"]["prompt"]
        assert "Ay max: 1" in preview["input_context"]["prompt"]
        assert "Contexte structure fourni par l'application" not in preview["input_context"]["prompt"]
        assert "Réponds uniquement avec ces balises" not in preview["input_context"]["prompt"]
        assert preview["input_context"]["context"]["stat_values"]["stat.vehicle.ay.max"] == 1
        assert "{{stat.vehicle.ay.max}}" not in preview["input_context"]["prompt"]
        ...
        assert "{{stat.vehicle.ay.max}}: 1" in prediction["analysis_text"]
        assert prediction["input_context"]["segment_tag_values"]["segment.label"] == "freinage"
        assert prediction["input_context"]["stat_values"]["stat.vehicle.ay.max"] == 1
