...
def test_root_serves_ui_html():
    ...
    assert "<title>DynaScope</title>" in html
    ...
    assert "Analyse qualité" in html
    assert 'data-view="quality"' in html
    assert 'id="view-quality"' in html
    assert 'id="qualitySteeringChannelSelect"' in html
    assert 'id="qualityLabelSelect"' in html
    assert 'id="qualitySteeringDriftPlot"' in html
