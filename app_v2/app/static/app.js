...
const STEERING_CHANNEL_PATTERNS = [
  "steer",
  "wheelsteer",
  "whldir",
  "braqu",
...
const DYNAMIC_STAT_DEFINITIONS = [
  { name: "min", label: "Min" },
  { name: "max", label: "Max" },
  { name: "mean", label: "Moyenne" },
  { name: "std", label: "Std" },
  { name: "rms", label: "RMS" },
  { name: "amplitude", label: "Amplitude" },
];
const DYNAMIC_CONTEXT_TAGS = [
  { tag_name: "segment.label", label: "Label annotation" },
];
const DEFAULT_DYNAMIC_SYSTEM_PROMPT = `Tu es un expert en analyse dynamique véhicule et en exploitation d'essais.

Tu reçois :
- un artefact visuel contenant les signaux sélectionnés ;
- le label du segment annoté : {{segment.label}} ;
- les statistiques quantitatives que tu as insérées dans ce prompt via les balises disponibles.

Objectif :
produire une interprétation technique du segment annoté.

Règles :
- base ton analyse uniquement sur les signaux, statistiques et métadonnées fournis ;
- cite explicitement les canaux ou statistiques utilisés ;
- distingue les observations directes des hypothèses ;
- si une information manque, indique-le clairement ;
- ne produis pas de diagnostic définitif si les données ne le permettent pas.

Réponds strictement avec les balises suivantes :

<analyse>
Analyse technique détaillée du segment.
</analyse>

<note_analyse>
Points de vigilance, limites, valeurs remarquables, incohérences éventuelles.
</note_analyse>

<synthese>
Synthèse courte exploitable par un analyste.
</synthese>`;
...
function renderGlobalCampaignStatus() {
  const container = $("globalCampaignStatus");
  if (!container) return;
  const analysis = state.analysis;
  if (!analysis) {
    container.className = "campaign-status empty";
    container.innerHTML = `
      <span class="campaign-status-dot" aria-hidden="true"></span>
      <span class="campaign-status-name">Aucune sélection</span>
    `;
    return;
  }

  const readiness = analysis.readiness || {};
  const statusClass = readiness.ready ? "ready" : "incomplete";

  container.className = `campaign-status ${statusClass}`;
  container.innerHTML = `
    <span class="campaign-status-dot" aria-hidden="true"></span>
    <span class="campaign-status-name">${escapeHtml(analysis.name || "-")}</span>
  `;
}
...
function renderMarkdownLite(text) {
  ...
  return lines.map(line => {
    if (line.startsWith("### ")) return `<h3>${renderMarkdownInline(line.slice(4))}</h3>`;
    if (line.startsWith("## ")) return `<h2>${renderMarkdownInline(line.slice(3))}</h2>`;
    if (line.startsWith("- ")) return `<div>• ${renderMarkdownInline(line.slice(2))}</div>`;
    if (!line.trim()) return "<br>";
    return `<p>${renderMarkdownInline(line)}</p>`;
  }).join("");
}

function markdownSectionBlock(title, text) {
  const content = String(text || "").trim();
  return `
    <article class="analysis-section">
      <h3>${escapeHtml(title)}</h3>
      <div>${content ? renderMarkdownLite(content) : '<div class="muted">Non renseigné.</div>'}</div>
    </article>
  `;
}

function renderDynamicInterpretation(run) {
  if (!run) return '<div class="muted">Aucune prédiction générée pour cette annotation.</div>';
  const response = run.response_json || {};
  const summary = response.synthese || run.summary_text || "";
  const analysis = response.analyse || run.analysis_text || "";
  const note = response.note_analyse || run.analysis_note || "";
  if (summary || analysis || note) {
    return `
      <div class="analysis-sections">
        ${markdownSectionBlock("Synthèse", summary)}
        ${markdownSectionBlock("Analyse", analysis)}
        ${markdownSectionBlock("Note d'analyse", note)}
      </div>
    `;
  }
  return renderMarkdownLite(run.response_markdown || "");
}
...
function resizeVisiblePlotsSoon() {
  requestAnimationFrame(() => {
    resizePlots(["explorationSignalPlot", "explorationTrajectoryPlot", "parametricPlot", "qualitySteeringDriftPlot", "annotationSignalPlot", "annotationTrajectoryPlot", "dynamicSegmentSignalPlot"]);
    setTimeout(() => {
      resizePlots(["explorationSignalPlot", "explorationTrajectoryPlot", "parametricPlot", "qualitySteeringDriftPlot", "annotationSignalPlot", "annotationTrajectoryPlot", "dynamicSegmentSignalPlot"]);
    }, 120);
  });
...
function isSteeringChannel(channel) {
  const text = `${channel?.value || ""} ${channel?.label || ""}`.toLowerCase();
  return STEERING_CHANNEL_PATTERNS.some(pattern => text.includes(pattern));
}

function renderQualityChannelOptions(previous = "") {
  const select = $("qualitySteeringChannelSelect");
  if (!select) return;
  const steeringChannels = state.qualityChannels.filter(isSteeringChannel);
  const channels = steeringChannels.length ? steeringChannels : state.qualityChannels;
  select.innerHTML = channels.length
    ? channels.map(channel => `
      <option value="${escapeHtml(channel.value)}">${escapeHtml(channel.label || channel.value)}</option>
    `).join("")
    : '<option value="">Aucun canal de braquage détecté</option>';
  const wanted = previous || channels[0]?.value || "";
  select.value = channels.some(channel => channel.value === wanted) ? wanted : (channels[0]?.value || "");
}

function renderQualityLabelOptions() {
  const select = $("qualityLabelSelect");
  if (!select) return;
  const previous = select.value;
  const labels = state.annotationLabels || [];
  select.innerHTML = [
    '<option value="">Tous les labels</option>',
    ...labels.map(label => `<option value="${escapeHtml(label)}">${escapeHtml(label)}</option>`),
  ].join("");
  const straightLabel = labels.find(label => String(label).toLowerCase().includes("ligne droite"));
  const wanted = previous && labels.includes(previous) ? previous : (straightLabel || "");
  select.value = wanted;
}
...
function signalPlotTraces(payload) {
  return (payload.channels || []).map((channel, index) => {
    ...
    const points = time
      .map((x, pointIndex) => ({
        x: Number(x),
        y: values[pointIndex] == null ? null : Number(values[pointIndex]),
      }))
    ...
    return {
      ...
      yaxis: index === 1 ? "y2" : "y",
      line: { color: ["#1368ce", "#15803d", "#e26d0a", "#8b5cf6", "#b42318"][index % 5], width: 1.5 },
      hovertemplate: "t=%{x:.2f}s<br>val=%{y:.4g}<extra></extra>",
    };
  }).filter(trace => trace.x.length && trace.y.length);
}

function drawSignalPayloadPlot(plotId, payload, title, shapes = []) {
  if (!window.Plotly) return;
  const traces = signalPlotTraces(payload);
  if (!traces.length) {
    drawEmptyPlot(plotId, "Aucun signal");
    return;
  }
...
function drawSignalPlot(payload) {
  drawSignalPayloadPlot("explorationSignalPlot", payload, "Signaux rééchantillonnés");
}
...
function drawQualitySteeringBoxplot(payload) {
  if (!window.Plotly) return;
  const rows = payload.items || [];
  if (!rows.length) {
    drawEmptyPlot("qualitySteeringDriftPlot", "Aucune boîte temporelle disponible");
    return;
  }
  const periodOrder = [...new Set(rows.map(row => row.time_bucket).filter(Boolean))];
  const xIndex = new Map(periodOrder.map((period, index) => [period, index]));
  const shapes = [];
  const hoverX = [];
  const hoverY = [];
  const hoverText = [];
  rows.forEach((row, index) => {
    const x = xIndex.has(row.time_bucket) ? xIndex.get(row.time_bucket) : index;
    const color = "#1368ce";
    const fill = "rgba(19,104,206,0.18)";
    const x0 = x - 0.24;
    const x1 = x + 0.24;
    shapes.push(
      { type: "line", xref: "x", yref: "y", x0: x, x1: x, y0: row.q05, y1: row.q95, line: { color, width: 1.5 } },
      { type: "line", xref: "x", yref: "y", x0: x - 0.16, x1: x + 0.16, y0: row.q05, y1: row.q05, line: { color, width: 1.5 } },
      { type: "line", xref: "x", yref: "y", x0: x - 0.16, x1: x + 0.16, y0: row.q95, y1: row.q95, line: { color, width: 1.5 } },
      { type: "rect", xref: "x", yref: "y", x0, x1, y0: row.q25, y1: row.q75, fillcolor: fill, line: { color, width: 1.8 } },
      { type: "line", xref: "x", yref: "y", x0, x1, y0: row.q50, y1: row.q50, line: { color: "#17202c", width: 2 } }
    );
    hoverX.push(x);
    hoverY.push(row.q50);
    hoverText.push([
      row.time_bucket,
      `q05=${formatNumber(row.q05)}`,
      `q25=${formatNumber(row.q25)}`,
      `q50=${formatNumber(row.q50)}`,
      `q75=${formatNumber(row.q75)}`,
      `q95=${formatNumber(row.q95)}`,
      `${row.n_points} points | ${row.n_files} fichier(s)`,
    ].join("<br>"));
  });
  const traces = [{
    type: "scatter",
    mode: "markers",
    name: payload.filters?.signal || "braquage",
    x: hoverX,
    y: hoverY,
    text: hoverText,
    marker: { size: 10, color: "rgba(19,104,206,0.01)" },
    hovertemplate: "%{text}<extra></extra>",
    showlegend: false,
  }];
  const layout = plotlyLayout("Distribution temporelle du braquage", payload.filters?.signal || "braquage");
  layout.xaxis.title = "tranche temporelle";
  layout.xaxis.type = "linear";
  layout.xaxis.tickmode = "array";
  layout.xaxis.tickvals = periodOrder.map((_, index) => index);
  layout.xaxis.ticktext = periodOrder;
  layout.xaxis.tickangle = -35;
  layout.shapes = shapes;
  Promise.resolve(Plotly.react("qualitySteeringDriftPlot", traces, layout, plotlyConfig()))
    .then(() => resizePlotSoon("qualitySteeringDriftPlot"));
}
...
async function saveDynamicPrompt() {
  ...
  const payload = {
    ...
    indicators: selectedDynamicStatTags(),
...
async function runDynamicAnalysis() {
  ...
  const result = await api(`/api/analyses/${encodeURIComponent(analysisId)}/dynamic-analysis/${encodeURIComponent(dynamicAnalysisId)}/predictions/${encodeURIComponent(annotationId)}`, {
    method: "POST",
    body: JSON.stringify(dynamicPredictionPayload()),
  });
  await loadDynamicAnalysis();
...
async function loadQualityFiles() {
  ...
  if (!analysisId) {
    state.qualityFiles = [];
    state.qualityFileId = "";
    renderQualityLabelOptions();
    return;
  }
  if (!state.annotationLabels.length) {
    await loadAnnotationLabels();
  }
  renderQualityLabelOptions();
  const payload = await api(`/api/analyses/${encodeURIComponent(analysisId)}/exploration/files`);
  state.qualityFiles = payload.items || [];
  state.qualityFileId = state.qualityFiles[0]?.file_id || "";
  if (state.qualityFileId) {
    await loadQualityChannels();
  } else {
    state.qualityChannels = [];
    renderQualityChannelOptions();
  }
}

async function loadQualityChannels() {
  const analysisId = currentAnalysisId();
  const fileId = state.qualityFileId || state.qualityFiles[0]?.file_id;
  if (!analysisId || !fileId) return;
  const previous = $("qualitySteeringChannelSelect")?.value || "";
  const payload = await api(`/api/analyses/${encodeURIComponent(analysisId)}/exploration/files/${encodeURIComponent(fileId)}/signals/options`);
  state.qualityChannels = (payload.items || []).filter(item => item.available);
  renderQualityChannelOptions(previous);
}
...
async function refreshQualitySteeringDrift() {
  const analysisId = currentAnalysisId();
  const channel = $("qualitySteeringChannelSelect")?.value;
  if (!analysisId) {
    setQualityStatus("Sélectionne une campagne avec des JSON exportés.");
    drawEmptyPlot("qualitySteeringDriftPlot", "Aucune donnée");
    return;
  }
  if (!state.qualityChannels.length) {
    await loadQualityChannels();
  }
  const selectedChannel = channel || $("qualitySteeringChannelSelect")?.value;
  if (!selectedChannel) {
    setQualityStatus("Aucun canal de braquage détecté dans le JSON sélectionné.");
    drawEmptyPlot("qualitySteeringDriftPlot", "Canal de braquage manquant");
    return;
  }
  const maxPoints = $("qualityMaxPointsInput")?.value || "250000";
  const boxParams = new URLSearchParams({
    signal: selectedChannel,
    time_grouping: $("qualityTimeGroupingSelect")?.value || "day_hour",
    min_points: $("qualityMinPointsInput")?.value || "10",
    max_points: maxPoints,
  });
  const label = $("qualityLabelSelect")?.value || "";
  if (label) boxParams.set("label", label);
  setQualityStatus("Chargement des distributions de braquage...");
  const boxplot = await api(`/api/analyses/${encodeURIComponent(analysisId)}/exploration/boxplot?${boxParams.toString()}`);
  drawQualitySteeringBoxplot(boxplot);
  setQualityStatus([
    `Canal: ${selectedChannel}`,
    `Label: ${label || "Tous"}`,
    `Boîtes temporelles: ${boxplot.items?.length || 0}`,
  ].join("\n"));
}
...
function dynamicStatTagName(sourceName, statName) {
  const safeSource = String(sourceName || "")
    .trim()
    .replace(/[^A-Za-z0-9_.-]+/g, "_")
    .replace(/^[_ .-]+|[_ .-]+$/g, "");
  return `stat.${safeSource}.${statName}`;
}

function selectedDynamicStatTags() {
  return Array.from(document.querySelectorAll('input[name="dynamic-stat-tag"]:checked')).map(input => ({
    source_kind: input.dataset.sourceKind || "channel",
    source_name: input.dataset.sourceName || "",
    stat_name: input.dataset.statName || "",
    tag_name: input.dataset.tagName || "",
    enabled: true,
  })).filter(item => item.source_name && item.stat_name && item.tag_name);
}
...
function insertDynamicStatTag(tagName) {
  insertTextAtCursor($("dynamicAnalysisSystemPromptInput"), `{{${tagName}}}`);
}

function showDynamicArtifactTab(tab) {
  const target = tab || "image";
  document.querySelectorAll("#dynamicArtifactTabs button").forEach(button => {
    button.classList.toggle("active", button.dataset.dynamicArtifactTab === target);
  });
  document.querySelectorAll(".dynamic-artifact-tab").forEach(panel => panel.classList.remove("active"));
  $(`dynamic-artifact-${target}`)?.classList.add("active");
  if (target === "image") resizeVisiblePlotsSoon();
}

function showDynamicInterpretationTab(tab) {
  const target = tab || "llm";
  document.querySelectorAll("#dynamicInterpretationTabs button").forEach(button => {
    button.classList.toggle("active", button.dataset.dynamicInterpretationTab === target);
  });
  document.querySelectorAll(".dynamic-interpretation-tab").forEach(panel => panel.classList.remove("active"));
  $(`dynamic-interpretation-${target}`)?.classList.add("active");
}
...
function resetDynamicAnalysisForm() {
  ...
  if ($("dynamicAnalysisSystemPromptInput")) {
    $("dynamicAnalysisSystemPromptInput").value = DEFAULT_DYNAMIC_SYSTEM_PROMPT;
  }
  ...
  if ($("dynamicAnalysisStatTagTable")) $("dynamicAnalysisStatTagTable").innerHTML = "";
  renderDynamicChannelOptions();
  renderDynamicLabelOptions();
  renderDynamicStatTagOptions();
  renderDynamicChoiceSummary();
  renderDynamicRuns();
...
function renderDynamicChannelOptions() {
  ...
  container.querySelectorAll('input[name="dynamic-channel"]').forEach(input => {
    input.addEventListener("change", renderDynamicStatTagOptions);
  });
  renderDynamicStatTagOptions();
}
...
function renderDynamicStatTagOptions(options = {}) {
  const container = $("dynamicAnalysisStatTagTable");
  if (!container) return;
  const dynamicAnalysis = selectedDynamicAnalysis();
  const previous = options.preferSaved
    ? new Set()
    : new Set(selectedDynamicStatTags().map(item => item.tag_name));
  const saved = new Set((dynamicAnalysis?.indicators || [])
    .filter(item => item?.enabled !== false)
    .map(item => item.tag_name)
    .filter(Boolean));
  const selectedChannels = selectedDynamicChannels();
  const channels = selectedChannels.length
    ? selectedChannels
    : (dynamicAnalysis?.selected_channels || []).slice();
  const rows = channels.flatMap(channel => DYNAMIC_STAT_DEFINITIONS.map(stat => {
    const tagName = dynamicStatTagName(channel, stat.name);
    return {
      source_kind: "channel",
      source_name: channel,
      stat_name: stat.name,
      ...
      tag_name: tagName,
      checked: previous.has(tagName) || (!previous.size && saved.has(tagName)),
    };
  }));
  renderTable(
    "dynamicAnalysisStatTagTable",
    [
      {
        key: "selected",
        label: "",
        render: row => `
          <input
            type="checkbox"
            name="dynamic-stat-tag"
            data-source-kind="${escapeHtml(row.source_kind)}"
            data-source-name="${escapeHtml(row.source_name)}"
            data-stat-name="${escapeHtml(row.stat_name)}"
            data-tag-name="${escapeHtml(row.tag_name)}"
            ${row.checked ? "checked" : ""}
          >
        `,
      },
      { key: "source_name", label: "Source" },
      { key: "stat_label", label: "Statistique" },
      { key: "tag_name", label: "Balise", render: row => `<code>{{${escapeHtml(row.tag_name)}}}</code>` },
      {
        key: "insert",
        label: "",
        render: row => `<button type="button" class="secondary dynamic-stat-insert" data-tag-name="${escapeHtml(row.tag_name)}">Insérer</button>`,
      },
    ],
    rows,
    "Sélectionne d'abord des canaux pertinents"
  );
  container.querySelectorAll(".dynamic-stat-insert").forEach(button => {
    button.addEventListener("click", () => insertDynamicStatTag(button.dataset.tagName || ""));
  });
}
...
function renderDynamicChoiceSummary() {
  ...
  node.innerHTML = `
    <div class="summary-grid">
      ...
      <div><div class="label">Balises stats</div><div class="value">${escapeHtml((item.indicators || []).filter(indicator => indicator?.enabled !== false).length)}</div></div>
...
function dynamicAnalysisPayload() {
  ...
  return {
    ...
    indicators: dynamicAnalysis?.indicators?.length ? dynamicAnalysis.indicators : selectedDynamicStatTags(),
    output_schema: dynamicOutputSchema(),
...
function drawDynamicSegmentSignalPlot(segment) {
  ...
  const time = signalContext.time || [];
  const channels = Object.entries(signalContext.channels || {}).map(([name, channel]) => ({
    name,
    unit: channel?.unit || "",
  ...
  const shapes = Number.isFinite(start) && Number.isFinite(end) && end > start
    ? [{
      type: "rect",
      xref: "x",
      yref: "paper",
    ...
    : [];
  drawSignalPayloadPlot(
    plotId,
    { time, channels },
    "Signaux et indicateurs du segment",
    shapes
  );
}

function artifactUrl(path, version = "") {
  const value = String(path || "");
  const marker = "app_v2/app/static";
  const index = value.indexOf(marker);
  if (index >= 0) {
    const url = `/static${value.slice(index + marker.length)}`;
...
function renderDynamicGenerationSegment(segment, prediction) {
  ...
  if (image) {
    const imageVersion = prediction?.updated_at || prediction?.created_at || prediction?.prediction_id || "";
    const url = artifactUrl(prediction?.input_artifact_path, imageVersion);
    if (url) {
      image.src = url;
  ...
  const promptPreview = $("dynamicPromptPreview");
  if (promptPreview) {
    promptPreview.textContent = prediction?.input_context?.prompt
      || (segment ? "Calcul des instructions..." : "")
      || "Aucune instruction générée.";
  }
}

async function loadDynamicSegmentPreview(annotationId) {
  const analysisId = currentAnalysisId();
  const dynamicAnalysisId = state.dynamicSelectedAnalysisId || $("dynamicAnalysisPromptSelect")?.value;
  if (!analysisId || !dynamicAnalysisId || !annotationId) return;
  const requestId = ++state.dynamicPreviewRequestId;
  try {
    const result = await api(`/api/analyses/${encodeURIComponent(analysisId)}/dynamic-analysis/${encodeURIComponent(dynamicAnalysisId)}/preview/${encodeURIComponent(annotationId)}`, {
      method: "POST",
      body: JSON.stringify(dynamicPredictionPayload()),
    });
    if (requestId !== state.dynamicPreviewRequestId || state.dynamicSelectedAnnotationId !== annotationId) return;
    renderDynamicGenerationSegment(result.item?.input_context?.segment, result.item);
  } catch (error) {
    if (requestId !== state.dynamicPreviewRequestId) return;
    const promptPreview = $("dynamicPromptPreview");
    if (promptPreview) {
      promptPreview.textContent = `Prévisualisation indisponible: ${error.message || String(error)}`;
    }
  }
}
...
function selectDynamicRun(runId, annotationId = "") {
  ...
  if (segment?.annotation_id) {
    loadDynamicSegmentPreview(segment.annotation_id);
  }
  $("dynamicAnalysisResult").innerHTML = renderDynamicInterpretation(run);
  $("dynamicAnalysisCorrectionInput").value = "";
...
function bindActions() {
  ...
  $("mainNav").addEventListener("click", event => {
    ...
    if (button.dataset.view === "quality") {
      wrapAction(async () => {
        await loadQualityFiles();
        await refreshQualitySteeringDrift();
      }, "Analyse qualité chargée")();
    }
  ...
  $("qualitySteeringChannelSelect")?.addEventListener("change", wrapAction(refreshQualitySteeringDrift, "Analyse qualité chargée"));
  ["qualityLabelSelect", "qualityTimeGroupingSelect", "qualityMinPointsInput", "qualityMaxPointsInput"].forEach(id => {
    $(id)?.addEventListener("change", wrapAction(refreshQualitySteeringDrift, "Analyse qualité chargée"));
  });
  ...
  $("dynamicAnalysisPromptSelect")?.addEventListener("change", () => {
    ...
    renderDynamicStatTagOptions({ preferSaved: true });
    $("saveDynamicPromptBtn").textContent = "Créer l'analyse dynamique";
    wrapAction(async () => {
      const analysisId = currentAnalysisId();
  ...
  $("insertDynamicLabelTagBtn")?.addEventListener("click", () => insertDynamicStatTag("segment.label"));
  ...
  $("dynamicArtifactTabs")?.addEventListener("click", event => {
    const button = event.target.closest("button[data-dynamic-artifact-tab]");
    if (!button) return;
    showDynamicArtifactTab(button.dataset.dynamicArtifactTab);
  });
  $("dynamicInterpretationTabs")?.addEventListener("click", event => {
    const button = event.target.closest("button[data-dynamic-interpretation-tab]");
    if (!button) return;
    showDynamicInterpretationTab(button.dataset.dynamicInterpretationTab);
  });
  ...
  document.addEventListener("keydown", event => {
    ...
    if (activeView.id === "view-quality") return;
