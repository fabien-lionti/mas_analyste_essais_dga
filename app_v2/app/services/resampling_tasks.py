...
from app_v2.app.services.resampling_service import export_analysis_resampled_json
...
class ResamplingTaskManager:
    def __init__(self) -> None:
        self._tasks: dict[str, ResamplingTask] = {}
        self._lock = threading.Lock()

    def start(self, *, db_path: Path, analysis_id: str, options: dict[str, Any]) -> ResamplingTask:
        task = ResamplingTask(
            task_id=f"resampling_task_{uuid.uuid4().hex}",
    ...
    def get(self, task_id: str) -> ResamplingTask | None:
        with self._lock:
            return self._tasks.get(task_id)

    def latest_for_analysis(self, analysis_id: str) -> ResamplingTask | None:
        with self._lock:
    ...
    def cancel(self, task_id: str) -> ResamplingTask | None:
        task = self.get(task_id)
        if task is not None:
            task.cancel()
        return task

    def _run(self, task: ResamplingTask) -> None:
        task.update({"status": "running", "phase": "opening_db", "message": "Ouverture de la base"})
        try:
            with connect(task.db_path) as conn:
                init_db(conn)
                result = export_analysis_resampled_json(
                    conn,
            ...
            task.update(
                {
                    ...
                    "phase": result["status"],
                    "processed_files": result.get("exported_files", 0) + result.get("error_count", 0),
                    "total_files": result.get("total_files", 0),
                    "percent": 100 if result["status"] != "cancelled" else task.percent,
                    "message": "Export JSON terminé" if result["status"] != "cancelled" else "Export JSON interrompu",
                }
            )
            with task._lock:
                task.result = result
            ...
            with task._lock:
                ...
                task.phase = "failed"
                task.error = str(exc)
                task.message = "Export JSON échoué"
                task.updated_at = now_iso()


resampling_tasks = ResamplingTaskManager()
